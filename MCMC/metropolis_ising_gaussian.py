...
def run_metropolis_with_noise(image):
    ...
    for beta in beta_range:
        ...
        images_processing.save_image(
            'metropolis_noise_ising_beta={0}_iter={1}'.format(beta, iterations), 'jpeg',
            sampled_image,
            'Denoised images/metropolis_sampler_noise_ising_model/gaussian={0}'.format(SIGMA))


if __name__ == '__main__':
    # start = datetime.datetime.now()
    image = cv2.imread('Noisy images/gaussian/sigma=2/chest_gauss_sigma=2.jpeg')
